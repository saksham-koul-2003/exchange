...
export const MarketBar = ({market}: {market: string}) => {
    ...
    useEffect(() => {
       ...
       //you are maintaing a ws connnecton whenever server return u a ticker update me
       SignalingManager.getInstance().registerCallback("ticker", (data: Partial<Ticker>)  =>  setTicker(prevTicker => ({
        ...
        return () => {
            ...
            SignalingManager.getInstance().sendMessage({"method":"UNSUBSCRIBE","params":[`ticker.${market}`]});//cleanup logic first will run, if the market 
                                                                                                                   //changes again or the component unmopunts
        }
    }, [market])
    // 
